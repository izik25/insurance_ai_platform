"""Fetch Clal's health/life listing once, then download and populate the
DB from that same listing (same pattern as sync_phoenix.py).
...
doesn't mean re-running the search. Pass --refresh-listing to force a
fresh search (e.g. to pick up newly published documents).

Usage: python scripts/sync_clal.py [--limit N] [--refresh-listing]
"""
...
from companies.clal import register  # noqa: E402
from companies.clal.downloader import ClalDocumentRef, ClalDownloader  # noqa: E402
...
def _load_cached_listing(cache_path: Path) -> list[ClalDocumentRef] | None:
    if not cache_path.is_file():
        return None
    raw = json.loads(cache_path.read_text(encoding="utf-8"))
    return [ClalDocumentRef(**entry) for entry in raw]


def _save_listing_cache(cache_path: Path, refs: list[ClalDocumentRef]) -> None:
    payload = [
        {
            "domain": ref.domain,
            "title": ref.title,
...
def main() -> None:
    ...
    assert isinstance(plugin.downloader, ClalDownloader)

    destination = settings.raw_documents_dir / "clal"
