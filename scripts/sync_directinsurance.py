"""Fetch Direct Insurance's forms/policy listing once, then download and
populate the DB from that same listing (same pattern as sync_menorah.py).
...
doesn't mean re-querying every salesGroup. Pass --refresh-listing to force
a fresh search (e.g. to pick up newly published documents).

Usage: python scripts/sync_directinsurance.py [--limit N] [--refresh-listing]
"""
...
from companies.directinsurance import register  # noqa: E402
from companies.directinsurance.downloader import (  # noqa: E402
...
def _load_cached_listing(cache_path: Path) -> list[DirectInsuranceDocumentRef] | None:
    if not cache_path.is_file():
        return None
    raw = json.loads(cache_path.read_text(encoding="utf-8"))
    return [DirectInsuranceDocumentRef(**entry) for entry in raw]


def _save_listing_cache(cache_path: Path, refs: list[DirectInsuranceDocumentRef]) -> None:
    payload = [
        {
            "domain": ref.domain,
            "form_id": ref.form_id,
...
def main() -> None:
    ...
    assert isinstance(plugin.downloader, DirectInsuranceDownloader)

    destination = settings.raw_documents_dir / "directinsurance"
