"""Fetch Ayalon's whole-archive listing once, then download and populate the
DB from that same listing (same pattern as sync_menorah.py/sync_clal.py).
...
doesn't mean re-hitting the (bot-protected) archive page. Pass
--refresh-listing to force a fresh search (e.g. to pick up newly published
documents).

Usage: python scripts/sync_ayalon.py [--limit N] [--refresh-listing]
"""
...
from companies.ayalon import register  # noqa: E402
from companies.ayalon.downloader import AyalonDocumentRef, AyalonDownloader  # noqa: E402
...
def _load_cached_listing(cache_path: Path) -> list[AyalonDocumentRef] | None:
...
def _save_listing_cache(cache_path: Path, refs: list[AyalonDocumentRef]) -> None:
    payload = [
        {
            "domain": ref.domain,
            "title": ref.title,
...
def main() -> None:
    ...
    assert isinstance(plugin.downloader, AyalonDownloader)

    destination = settings.raw_documents_dir / "ayalon"
