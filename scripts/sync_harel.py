"""Fetch Harel's health/life listing once, then download and populate the
...
doesn't mean re-paginating every area. Pass --refresh-listing to force a
fresh listing (e.g. to pick up newly published documents).

Usage: python scripts/sync_harel.py [--limit N] [--refresh-listing]
"""
...
from companies.harel import register  # noqa: E402
from companies.harel.downloader import HarelDocumentRef, HarelDownloader  # noqa: E402
...
def _load_cached_listing(cache_path: Path) -> list[HarelDocumentRef] | None:
...
def _save_listing_cache(cache_path: Path, refs: list[HarelDocumentRef]) -> None:
    payload = [
        {
            "domain": ref.domain,
            "title": ref.title,
...
def main() -> None:
    ...
    arg_parser.add_argument(
        "--refresh-listing", action="store_true", help="Force a fresh listing of the archive."
    )
    ...
    register(registry)
    plugin = registry.get("harel")
    assert isinstance(plugin.downloader, HarelDownloader)

    destination = settings.raw_documents_dir / "harel"
