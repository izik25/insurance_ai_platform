...
- מקור: `555.co.il/webapp/api/siteapi/form/` — קישור חיים (`life_toolsactions.html`) וקישור בריאות (`health_toolsactions.html`) נשלחו בנפרד, שניהם בפנים כעת. **הפשוט ביותר מבין 5 החברות — אין הגנת בוט בכלל, אין דפדפן/session בכלל**: `GET /formdata` מחזיר את כל הטקסונומיה (products, salesGroup לכל product, formTypesActive לכל salesGroup) בקריאה אחת; `POST /sendformdata` עם `{"product","saleGroup","formType":[...],"active":"false"}` (`formType` הוא מערך — כל סוגי הטופס הקיימים לקטגוריה בבקשה אחת) מחזיר את כל התוצאות; `GET /openform/{formId}` מוריד את הקובץ ישירות — כל שלושת הקריאות אושרו חי כ-`httpx` רגיל, בלי cookies/session. `DOMAIN_TO_PRODUCT` ב-`config.py`: life→product 7, health→product 8.
...
- **סיווג domain**: שתי תתי-קטגוריות תחת עמוד ה"חיים" ("אובדן כושר עבודה", "נכות מתאונה") הן בפועל תוכן דמוי-בריאות (נכות/אובדן כושר עבודה) שמגדל ומנורה כבר מסווגות כ-health — הועלה לדיון מול המשתמש, אך **הוחלט במפורש להשאיר אותן domain="life"** (כפי שהאתר עצמו מקטלג אותן, תחת product=7) ולא להוסיף מורכבות של מיפוי per-salesGroup; המשתמש ציין שה-matching ימצא את ההתאמה הרלוונטית ממילא בלי צורך בזה.
- **אין שדה מספר נספח מובנה** (בניגוד לפניקס/כלל/מנורה) — `formName` בפורמט "<שם> <קוד>/<mahadura> - מהדורה <תאריך>" (למשל "195/01") או "נספח לקבלת שירותים..." (שימוש גנרי במילה "נספח" כתיאור קטגוריה, לא כ"נספח <מספר>"); `find_appendix_numbers` מופעל בכל זאת (הגנתי) אבל מחזיר [] כמעט תמיד — רוב מספרי הנספח מגיעים מ-backfill של ה-LLM בשלב `extract_documents.py` (קורא בפועל את תוכן ה-PDF), לא מהמטא-דאטה של האתר.
...
- **באג אמיתי שנתפס ותוקן**: `sync_directinsurance.py` בנה מחדש את שורת ה-`Document` בכל הרצה (כולל `--refresh-listing` על מסמכים שכבר קיימים) עם `appendix_number=ref.appendix_numbers` — וכיוון שכמעט תמיד ריק במטא-דאטה של האתר הזה (בניגוד לשאר החברות), `session.merge()` דרס בשקט את הערך שכבר עבר backfill מה-LLM בהרצה קודמת. אותר מיד אחרי ריצת `--refresh-listing` לבריאות (219→0 מסמכים עם appendix_number). תוקן ב-2 שכבות: (1) `raw_extraction` (JSONB, כבר שמור בכל `DocumentExtraction`) מכיל את הפלט המלא של ה-LLM כולל appendix_number/appendix_name — שימש לשחזור המידע בלי לקרוא ל-LLM שוב; (2) `sync_directinsurance.py` תוקן לשמר ערך appendix_number קיים ולא-ריק אם הרשימה הטרייה מחזירה רשימה ריקה, באותו עיקרון "אמון בגוף המקור, לא בניחוש" שכבר קיים ב-`_backfill_appendix_metadata`. **הערה לעתיד**: אותו דפוס merge קיים גם ב-`sync_menorah.py`/`sync_clal.py`/`sync_phoenix.py`, אבל שם appendix_number כמעט תמיד לא-ריק כבר מהמטא-דאטה של האתר עצמו, כך שה-backfill כמעט אף פעם לא רץ ואין מה לדרוס בפועל — לא תוקן שם כי לא רלוונטי, לא כי זה בטוח באופן עקרוני.
- **231 מסמכים ב-DB** (35 life / 196 health), 230 חולצו בהצלחה (1 נכשל — LEAD format), 216 עם embedding (14 טפסים אדמיניסטרטיביים חולצו עם שדות ריקים לגמרי — `embed_documents.py` מדלג נכון על טקסט ריק, כי אין להם תוכן כיסוי להשוות), 219/231 עם appendix_number לא ריק (94.8% כיסוי, רובו backfill מה-LLM), 458 התאמות חוצות-חברות (411 auto_confirmed / 47 pending_review).
...
1. **חברות נוספות** — מודולרי, לפי אותו pattern (`companies/<name>/`); מגדל+הפניקס+כלל+מנורה+ביטוח ישיר (חיים+בריאות) כבר בפנים, כל מסמך חדש עובר באותו pipeline חילוץ+embedding+matching בלי שום שינוי קוד.
2. **בדיקת איכות ה-matching** — לעבור בדשבורד על מדגם מהתאמות ולוודא שהן הגיוניות, כולל שימוש בציון "מי עדיף" (כלל אצבע חינמי, ר' `frontend/src/scoring.ts`) לבדיקת סבירות.
...
5. **סיכון ידוע, לא תוקן**: `sync_menorah.py`/`sync_clal.py`/`sync_phoenix.py` חולקים את אותו דפוס merge שגרם לבאג ב-`sync_directinsurance.py` (ר' סעיף ביטוח ישיר) — appendix_number מהרשימה הטרייה יכול לדרוס backfill קודם מה-LLM. לא תוקן שם כי appendix_number אצלן כמעט תמיד לא-ריק כבר מהמטא-דאטה של האתר עצמו, כך שבפועל אין מה לדרוס — אבל אם זה ישתנה (למשל תיווסף חברה חדשה עם מטא-דאטה חלקי), כדאי ליישם את אותו תיקון (שימור ערך קיים כשה-ref ריק).
